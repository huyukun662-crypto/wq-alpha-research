...
ROLL_DAYS = 3
IVTS_WIN = 250       # 个券隐含波动率的自身历史窗口
RV_WIN = 250         # 正股已实现波动率窗口(长口径,分母更稳)
TSVAL_WIN = 500      # 个券溢价率的自身历史窗口
...
class SrAlpha(CVBondAlphaBase):
    name = 'option__cb_iv_ts_blend'
    description = 'option cheapness vs own IV history, vs long realised vol, and premium vs own history'
    alpha_definition = ('0.50*z(neut(-rank(zscore(IV, 250)))) '
                        '+ 0.25*z(neut(-rank(IV - RV250))) '
                        '+ 0.25*z(neut(-rank(zscore(conv_prem, 500))))')
    ...
    def generate(self, start_date: datetime.datetime, end_date: datetime.datetime, is_inc=False):
        ...
        iv_s = pd.Series(iv, index=df.index)
        rv = df.groupby('b_sym')['s_log_ret'].transform(
            lambda x: x.rolling(RV_WIN, min_periods=RV_WIN // 2).std()) * np.sqrt(252)
        df['ivrv'] = -df.assign(_v=iv_s - rv).groupby('date')['_v'].transform(_cs_rank)

        # ---- 腿 0:期权贵贱的时序版(主力腿)----
        # 截面上比 IV-RV 要跨券比较,受个券固定效应干扰且变化快。改问「这只券的期权
        # 现在比它自己平时便宜多少」,把个券固定效应差掉。单腿即通过全部四项闸门:
        # OOS IC 0.0242 / ICIR 3.97 / SH 3.57 / turnover 0.097。
        iv_mu = iv_s.groupby(df['b_sym']).transform(
            lambda x: x.rolling(IVTS_WIN, min_periods=IVTS_WIN // 2).mean())
        iv_sd = iv_s.groupby(df['b_sym']).transform(
            lambda x: x.rolling(IVTS_WIN, min_periods=IVTS_WIN // 2).std())
        df['ivts'] = -df.assign(_v=(iv_s - iv_mu) / iv_sd.replace(0, np.nan)) \
        ...
        parts = []
        for col, wgt in (('ivts', W_IVTS), ('ivrv', W_IVRV), ('tsval', W_TSVAL)):

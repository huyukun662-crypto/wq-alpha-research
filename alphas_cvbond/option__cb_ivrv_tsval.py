...
class SrAlpha(CVBondAlphaBase):
    name = 'option__cb_ivrv_tsval'
    description = 'implied-minus-realised vol and own-history premium deviation, price/liquidity neutral'
    alpha_definition = ('0.5*z(neut(-rank(IV - RV60))) + 0.5*z(neut(-rank(zscore(conv_prem, 250))))')
    ...
    def generate(self, start_date: datetime.datetime, end_date: datetime.datetime, is_inc=False):
        ...
        parts = []
        for col, wgt in (('ivrv', W_IVRV), ('tsval', W_TSVAL)):

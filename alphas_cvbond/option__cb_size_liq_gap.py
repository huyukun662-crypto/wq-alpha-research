...
# 不含估值腿。双溢价率成分在 2025H2-2026Q1 转负(半年 PnL -4.26%),
# 是完整版组合近期衰减的唯一来源;此版本把它整个去掉。
W_SIZE = 0.40
W_ILLIQ = 0.30
W_GAP = 0.30
...
class SrAlpha(CVBondAlphaBase):
    name = 'option__cb_size_liq_gap'
    description = 'valuation-free blend: outstanding balance, illiquidity, delta-residual'
    alpha_definition = ('0.4*z(-rank(log(remain_size))) '
                        '+ 0.3*z(rank(ts_mean(|b_ret|/b_amount,20))) '
                        '+ 0.3*z(-rank(ts_sum(b_ret - delta*s_ret,20)))')
    ...
    def generate(self, start_date: datetime.datetime, end_date: datetime.datetime, is_inc=False):
        ...
        # --- 成分 3:弹性残差(转债实际涨跌 vs 理论 delta * 正股涨跌 的累计偏离)---
        ...
        # 先各自截面 z 再加权,使权重可比
        df['blend'] = (W_SIZE * size.groupby(df['date']).transform(_cs_z)

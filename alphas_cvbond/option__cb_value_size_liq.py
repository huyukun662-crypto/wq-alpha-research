...
class SrAlpha(CVBondAlphaBase):
    name = 'option__cb_value_size_liq'
    description = 'four-source blend: dual premium, outstanding balance, illiquidity, delta-residual'
    alpha_definition = ('0.3*z(-(0.5*rank(floor_prem)+0.5*rank(conv_prem))) '
                        '+ 0.3*z(-rank(log(remain_size))) '
                        '+ 0.2*z(rank(ts_mean(|b_ret|/b_amount,20))) '

...
21. **稀疏事件因子会「假性同质」,而且会伪造出漂亮的假因子**:
    ...
    方向都反了,证明原来那个正 Sharpe measured 的根本不是炸板承接力。
    **稀疏 0/1 字段做截面 rank 前必须先查非零覆盖率**;
    覆盖率极低时要么条件化到子样本,要么根本不要做截面 rank。
21b. **条件化之后的 limit_seal_quality 反号且 |Sharpe| 4.15,但先别当宝**:
    该因子与库内所有因子相关性均 ≤0.21(不是反转的马甲),经济含义是
    「炸板越多的涨停股后续越强」。但 4.15 已经高于除 vwap_deviation 外的所有因子,
    这种量级第一反应应该是找 bug 而不是庆祝;而且事后翻方向本身就是数据窥探。
    **在 OOS(2023–26)独立验证方向稳定性之前,不进组合、不当结论。**
